...
//! [R-09-010] Verificacion Final + Expulsion Segura
//!
...
impl VerificationReport {
    ...
    pub fn print_summary(&self, json_mode: bool) {
        if json_mode {
...
pub fn verify_directory_structure(usb_mount_point: &Path) -> Result<VerificationReport> {
    ...
    for entry in fs::read_dir(usb_mount_point)? {
        ...
        if !file_name.starts_with("VOL_") {
            report.add_warning(format!(
                "Non-indexed root directory detected (ignored by legacy stereo): {}",
                file_name
            ));
            continue;
        }
...
pub fn pre_eject_verification(
    ...
    report.print_summary(json_mode);
    Ok(report)
}
...
///
/// Ejecuta en Linux:
/// 1. `sync` — fuerza volcado de buffers del kernel
/// 2. `umount` — desmonta el punto de montaje
/// 3. `udisksctl power-off` — desconecta el dispositivo USB (si es soportado)
#[cfg(target_os = "linux")]
