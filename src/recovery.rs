...
impl RecoveryManager {
    ...
    /// Calcula el SHA256 de un archivo en disco para verificar integridad
    ...
    fn target_path_for(index: usize, normalized_name: &str, usb_mount: &Path) -> PathBuf {
        let volume_index = (index / 50) + 1;
        let volume_folder = format!("VOL_{:02}", volume_index);
        usb_mount.join(volume_folder).join(normalized_name)
    }
    ...
    fn purge_orphan_zero_byte_files(&self, checkpoint: &CheckpointData) -> Result<usize> {
        let expected_paths: HashSet<PathBuf> = checkpoint
            .processed_files
            .iter()
            .map(|(idx, f)| Self::target_path_for(*idx, &f.normalized_name, &self.usb_mount))
            .collect();
    ...
    pub fn execute_recovery(&self, checkpoint_mgr: &mut CheckpointManager) -> Result<()> {
        ...
        for index in indices_to_recover {
            ...
            match normalizer::normalize_audio(&file_data.original_path, &bad_file_path) {
                Ok(_) => {
                    let usb_hash = Self::compute_sha256(&bad_file_path)?;
