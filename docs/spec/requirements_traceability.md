...
| R-02-008 | R-03 | Sanitización de Nombre de Archivo | Transformar cualquier entrada UTF-8 a solo ASCII, eliminar caracteres ilegales y forzar longitud total <=32 chars incluyendo prefijo secuencial (`NNN_`) y extensión `.mp3`. | `crates/lap-core/src/sanitizer.rs` (`sanitize_filename`, `add_sequential_prefix`) |
